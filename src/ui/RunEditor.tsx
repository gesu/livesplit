...
export class RunEditor extends React.Component<Props, State> {
      ...
      <div className="run-editor">
            ...
            <div className="info-table-row">
              ...
              <div className="info-table-cell">
                <TextBox
                  ...
                  label="Category"
                  list={["run-editor-category-list", categoryNames]}
                />
              </div>
            </div>
            <div className="info-table-row">
        ...
        <div className="bottom-section">
          ...
          <div className="editor-group">
            <div className="tab-bar">{this.renderTabButtons(tab)}</div>
  ...
  private renderSegmentListButtons(): JSX.Element {
    ...
    return (
      <div className="btn-group">
        <button onClick={(_) => this.insertSegmentAbove()}>Insert Above</button>
        <button onClick={(_) => this.insertSegmentBelow()}>Insert Below</button>
        <button
        ...
        <button onClick={(_) => this.addComparison()}>Add Comparison</button>
        <button onClick={(_) => this.importComparison()}>
          Import Comparison
        </button>
        <button onClick={(e) => otherButtonToggleMenu(e)}></button>
      </div>
    );
  }
  ...
  private renderRulesButtons(): JSX.Element {
    return <div className="btn-group">{this.renderAssociateRunButton()}</div>;
  }
  ...
  private renderVariablesButtons(): JSX.Element {
    return (
      <div className="btn-group">
        ...
        <button onClick={(_) => this.addCustomVariable()}>Add Variable</button>
      </div>
    );
  }
  ...
  private renderLeaderboardButtons(category: Option<Category>): JSX.Element {
      ...
      if (this.variableIsValidForCategory(variable, category)) {
        if (variable["is-subcategory"]) {
          ...
          if (currentFilterValue === undefined) {
            const runValue =
              this.state.editor.metadata.speedrun_com_variables[variable.name];
    ...
    return (
      <div className="btn-group">
        ...
        <button className="disabled">Submit Run</button>
        <button onClick={(_) => this.interactiveAssociateRunOrOpenPage()}>
          {this.state.editor.metadata.run_id !== ""
            ? "Open PB Page"
            : "Associate Run"}
          ...
          speedrunComVariables.push({
            ...
            value: {
              CustomCombobox: {
                ...
                list: [
                  "",
                  ...Object.values(variable.values.values).map((v) => v.label),
                ],
          ...
          <table className="table">
            <tbody className="table-body">
              <tr>
                <td>
                  <p>
                    There are currently no Speedrun.com variables or custom
        ...
        <SettingsComponent
          ...
          setValue={(index, value) => {
            ...
            if (index === regionOffset) {
              ...
              const key =
                speedrunComVariables[index - speedrunComVariablesOffset].text;
              if (stringValue !== "") {
      ...
      <table className="table run-editor-tab leaderboard-table">
        ...
        <tbody className="table-body">
          {leaderboard.map((run) => {
            ...
            const evenOdd =
              rowIndex % 2 === 0 ? "table-row-odd" : "table-row-even";
            ...
            if (this.expandedLeaderboardRows.get(rowIndex) === true) {
              ...
              if (
                ...
                const videoUri =
                  run.videos.links[run.videos.links.length - 1].uri;
                      ...
                      <tbody>
                        ...
                        {map(platform, (p) => (
                          <tr>
                            ...
                            <td>
                              {p}
                              {run.system.emulated ? " Emulator" : null}
                            </td>
                          </tr>
                        ))}
                      </tbody>
          ...
          {this.state.editor.segments.map((s, segmentIndex) => {
            ...
            return (
              ...
              >
                ...
                {this.state.editor.segments[segmentIndex].comparison_times.map(
                      ...
                      <input
                        ...
                        value={
                          ...
                          this.state.rowState.comparisonTimesChanged[
                            ...
                            ? this.state.rowState.comparisonTimes[
                                comparisonIndex
                              ]
                            : comparisonTime
                        }
  ...
  private async downloadSplits<T>(apiRun: Run<T>, apiUri: string) {
    ...
    try {
      ...
      run.with((run) => {
        ...
        if (newEditor !== null) {
          associateRun(newEditor, gameName, categoryName, apiRun);
          ...
          toast.error(
            "The downloaded splits are not suitable for being edited."
          );
        }
      });
    } catch (_) {
