...
export class SplitsSelection extends React.Component<Props, State> {
          ...
          {isOpened ? null : (
            <>
              ...
              <button
                ...
                onClick={(e) => exportButtonToggleMenu(e)}
              ></button>
            </>
          )}
        ...
        <button
          onClick={(_) => {
            ...
            if (run !== null) {
              ...
              toast.error(
                "You can't edit your run while the timer is running."
              );
            }
          }}
        >
  ...
  private async openSplits(key: number) {
    const isModified = this.props.timer.readWith((t) =>
      t.getRun().hasBeenModified()
    );
    if (
      isModified &&
      !confirm(
